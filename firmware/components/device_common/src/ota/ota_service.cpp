...
esp_err_t handleVersion(httpd_req_t* req) {
  ...
  for (size_t i = 0; i < sizeof(desc->app_elf_sha256); ++i) {
    snprintf(digest + i * 2, 3, "%02x", desc->app_elf_sha256[i]);
  }
  esp_ota_img_states_t state;
  const auto err = esp_ota_get_state_partition(esp_ota_get_running_partition(), &state);
  cJSON* info = cJSON_CreateObject();
  if (!info) return ESP_ERR_NO_MEM;
  cJSON_AddStringToObject(info, "project_name", desc->project_name);
  cJSON_AddStringToObject(info, "version", desc->version);
  cJSON_AddStringToObject(info, "idf_ver", desc->idf_ver);
  cJSON_AddStringToObject(info, "date", desc->date);
  cJSON_AddStringToObject(info, "time", desc->time);
  cJSON_AddStringToObject(info, "elf_sha256", digest);
  cJSON_AddStringToObject(info, "target", CONFIG_IDF_TARGET);
  cJSON_AddStringToObject(info, "ota_state", err != ESP_OK ? "untracked" : state == ESP_OTA_IMG_PENDING_VERIFY ? "pending_verify"
                                                                       : state == ESP_OTA_IMG_VALID            ? "valid"
                                                                                                               : "other");
  cJSON_AddNumberToObject(info, "uptime_seconds", esp_timer_get_time() / 1000000);
...
void confirmOtaBootIfPending() {
  ...
  if (esp_ota_get_state_partition(running, &ota_state) == ESP_OK &&
    ...
    // The application calls this after initializing its services.
