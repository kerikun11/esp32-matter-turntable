...
def compile_and_run(tmp: Path, source: str, *sources: Path, includes=()):
    ...
    subprocess.run(['g++', '-std=c++17', '-Wall', '-Wextra', '-Werror',
                    '-I' + str(TOOLS.parent / 'components/device_common/include'),
...
class WebTests(unittest.TestCase):
    ...
    def test_device_name_and_hostname_share_first_row(self):
        source = server.TEMPLATE.read_text(encoding="utf-8")
        for label in ("デバイス名", "ホスト名"):
            match = re.search(
                r"<label class=\"([^\"]+)\">\s*<span>" + label, source)
            self.assertIsNotNone(match)
            self.assertEqual(match.group(1), "setting-card")

    def test_simple_status_card_and_shared_pairing_layout(self):
        source = server.TEMPLATE.read_text(encoding="utf-8")
        for removed in ('<svg', 'class="dial', 'id="needle"', 'id="matter-badge"', 'Matter：'):
            self.assertNotIn(removed, source)
        self.assertIn('class="card status-card"', source)
...
''', includes=(MAIN, TOOLS.parent / 'components/device_common/include', tmp))
    ...
    def test_actual_cpp_hostname_validation(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            compile_and_run(tmp, '#include <cassert>\n#include "settings/servo_settings.h"\n'
                            f'int main() {{\n{cases}\n}}\n',
                            MAIN / 'settings/servo_settings.cpp', includes=(stubs, MAIN))
